...
data_table.text(.05, .65, 'Initial Bunch Length: {:.2f} [mm]'\
                .format(initial_bunchlength/mm), fontsize=10)
...
bunchlength_text = data_table.text(0.05, 0.60, "", transform=data_table.transAxes)
#lstpart_text = data_table.text(0.05, 0.70, "", transform=data_table.transAxes)
time_text = data_table.text(0.05, 0.55, "", transform=data_table.transAxes)
...
def init():
    ...
    return scat, com_scat, line, std_time_line, bunchlength_template, time_text
...
def animate(i):
   ...
   # else:
    ...
    #time coordinates
    time_point = np.array([tracked_data['time'][i]])/ms
    ...
    com_scat_point = np.hstack((time_point[:,np.newaxis], com_zpoint[:,np.newaxis])) #Particle plotting
    ...
    line.set_data(com_zpoints, stdz_zpoints) #std_z line plot
    ...
    #Update plots
    return scat, com_scat, line, std_time_line, bunchlength_text, time_text
...
plt.show()
ani.save(file_save + 'zE{:.4f}run.mp4'.format(particle_energy/kV), writer=writer)
