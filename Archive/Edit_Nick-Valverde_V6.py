...
#--Point to point load
#particle_energy_range = particle_energy*np.array([.95, .97, .99, 1, 1.01, 1.03, 1.05])
#for energy in particle_energy_range:
    #p = MyParticle(energy, uranium_beam)
    #load_list.append(p.loader('gaussian'))

#--Parallel to point load
x_list = np.array([i*.001 for i in range(-4, 4 + 1)])
for elem in x_list:
    p = MyParticle(particle_energy,uranium_beam)
    load_list.append(p.loader('gaussian', avg_coordinates = (elem, 0, 0), sigma = (0, 0, 0)))
